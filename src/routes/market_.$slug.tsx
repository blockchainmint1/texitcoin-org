...
import { getHitListCoin, getCmcQuote } from "@/lib/market.functions";
import { computeFakeScore } from "@/lib/fake-score";

const coinQuery = (slug: string) =>
  queryOptions({
    queryKey: ["hit-list-coin", slug],
    queryFn: () => getHitListCoin({ data: { slug } }),
    staleTime: 60_000,
  });

export const Route = createFileRoute("/market_/$slug")({
  loader: ({ params, context }) => context.queryClient.ensureQueryData(coinQuery(params.slug)),
  head: ({ loaderData, params }) => {
    const c = loaderData;
    if (!c) return { meta: [{ title: "Case study — TEXITcoin" }] };
    const url = `https://texitcoin.org/market/${params.slug}`;
    const title = `${c.name} (${c.symbol}) — Hit List case study | TEXITcoin`;
    const desc = c.verdictNote ?? `Does ${c.name} still belong in the CMC Top 100? An honest look at the numbers, the socials, and the argument.`;
    return {
      meta: [
        { title },
  ...
  notFoundComponent: () => (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="pt-40 pb-24 mx-auto max-w-3xl px-6 text-center">
        <h1 className="font-display text-5xl font-bold">Not on the hit list (yet)</h1>
        <p className="mt-4 text-muted-foreground">We haven't evaluated that one yet. Check back soon.</p>
        <Link to="/market" className="mt-8 inline-flex items-center gap-2 text-primary font-semibold">
          <ArrowLeft className="h-4 w-4" /> Back to the hit list
        </Link>
      </main>
      <Footer />
    </div>
...
function CoinCaseStudyPage() {
  const { slug } = Route.useParams();
  const { data: coin } = useSuspenseQuery(coinQuery(slug));
      ...
      <main>
        <article className="pt-32 pb-16">
          <div className="mx-auto max-w-4xl px-6">
            ...
            >
              <ArrowLeft className="h-4 w-4" /> Hit list
            </Link>

            {/* Verdict banner */}
            <div className={`mt-8 rounded-3xl border p-8 md:p-10 ${v.cls} ${v.ring}`}>
            ...
            <section className="mt-14">
              ...
              <h2 className="mt-3 font-display text-3xl font-bold">Live from CoinMarketCap</h2>
            ...
            {coin.gaps.length > 0 && (
              <section className="mt-16">
                ...
                <h2 className="mt-3 font-display text-3xl font-bold">Where the gap actually is</h2>
            ...
            <div className="mt-16 rounded-2xl border border-border bg-card p-8 shadow-card">
              <div className="text-xs uppercase tracking-[0.22em] text-primary font-semibold">Back to the list</div>
              <h3 className="mt-2 font-display text-2xl font-bold">See who else is on the hit list.</h3>
              <p className="mt-3 text-muted-foreground">Every entry is a full case study. We're evaluating every coin in the CMC Top 100 — one at a time, honestly.</p>
              <Link
                to="/market"
